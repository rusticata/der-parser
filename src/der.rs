...
impl<'a> DerObjectContent<'a> {
    pub fn as_u64(&self) -> Result<u64, DerError> {
        match *self {
            DerObjectContent::Integer(i) => bytes_to_u64(i).or(Err(DerError::IntegerTooLarge)),
    ...
    pub fn tag(&self) -> DerTag {
        match *self {
            ...
            DerObjectContent::ContextSpecific(_, _) | DerObjectContent::Unknown(_) => {
                DerTag::Invalid
            }
        }
    }
