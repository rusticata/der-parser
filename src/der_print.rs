use crate::der::{BitStringObject, DerObject, DerObjectContent};
use std::fmt;
use std::str;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if self.flags.contains(&PrettyPrinterFlag::ShowHeader) {
            r#try!(write!(
                f,
                "[c:{}, s:{}, t:{}] ",
                self.obj.class, self.obj.structured, self.obj.tag
            ));
        };
        ...
        match self.obj.content {
            ...
            DerObjectContent::OctetString(v) => {
                writeln!(f, "OctetString({:?})", debug::HexSlice { d: v })
            }
            DerObjectContent::BitString(u, BitStringObject { data: v }) => {
                writeln!(f, "BitString({},{:?})", u, debug::HexSlice { d: v })
            }
            DerObjectContent::GeneralizedTime(s) => {
                print_utf8_string_with_type(f, s, "GeneralizedTime")
            }
            DerObjectContent::UTCTime(s) => print_utf8_string_with_type(f, s, "UTCTime"),
            DerObjectContent::PrintableString(s) => {
                print_utf8_string_with_type(f, s, "PrintableString")
            }
            DerObjectContent::NumericString(s) => {
                print_utf8_string_with_type(f, s, "NumericString")
            }
            ...
            DerObjectContent::GeneralString(s) => {
                print_utf8_string_with_type(f, s, "GeneralString")
            }
            ...
            DerObjectContent::Set(ref v) | DerObjectContent::Sequence(ref v) => {
                let ty = if self.obj.tag == 0x10 {
                    "Sequence"
                } else {
                    "Set"
                };
