        ...
        Ok((rem, ref obj)) => match obj.content {
            DerObjectContent::Integer(i) => match i.len() {
                ...
                _ => Err(Err::Error(error_position!(
                    i,
                    ErrorKind::Custom(DER_INTEGER_TOO_LARGE)
                ))),
            },
            _ => Err(Err::Error(error_position!(
                i,
                ErrorKind::Custom(DER_TAG_ERROR)
            ))),
        },
        Err(e) => Err(e),
    }
...
pub fn der_read_element_content_as(
    ...
    match tag {
        ...
        0x02 => map!(i, take!(len), |i| DerObjectContent::Integer(i)),
        // 0x03: bitstring
        0x03 => {
            do_parse!(
                i,
        ...
        // 0x0a: enumerated
        0x0a => map!(i, parse_hex_to_u64!(len), |i| DerObjectContent::Enum(i)),
        // 0x0c: UTF8String
        0x0c => {
            map!(
                i,
        ...
        // 0x10: sequence
        0x10 => map!(i, flat_take!(len, der_read_sequence_content), |l| {
            DerObjectContent::Sequence(l)
        }),
        // 0x11: set
        0x11 => map!(i, flat_take!(len, der_read_set_content), |l| {
            DerObjectContent::Set(l)
        }),
        // 0x12: numericstring
        0x12 => {
            map!(
        ...
        _ => Err(Err::Error(error_position!(
            i,
            ErrorKind::Custom(DER_TAG_UNKNOWN)
        ))),
    }
}
...
pub fn der_read_element_content(i: &[u8], hdr: DerObjectHeader) -> IResult<&[u8], DerObject> {
    ...
    match der_read_element_content_as(i, hdr.tag, hdr.len as usize) {
        ...
        Err(Err::Error(Context::Code(_, ErrorKind::Custom(DER_TAG_UNKNOWN)))) => map!(
            i,
            take!(hdr.len),
            |b| DerObject::from_header_and_content(hdr, DerObjectContent::Unknown(b))
        ),
...
pub fn parse_der_enum(i: &[u8]) -> IResult<&[u8], DerObject> {
    do_parse!(
        ...
        hdr: der_read_element_header
            >> error_if!(
                hdr.tag != DerTag::Enumerated as u8,
                ErrorKind::Custom(DER_TAG_ERROR)
            )
...
pub fn parse_der_relative_oid(i: &[u8]) -> IResult<&[u8], DerObject> {
    do_parse!(
        ...
        hdr: der_read_element_header
            >> error_if!(
                hdr.tag != DerTag::RelativeOid as u8,
                ErrorKind::Custom(DER_TAG_ERROR)
            )
            >> content: map_res!(take!(hdr.len), der_read_relative_oid)
            >> (DerObject::from_header_and_content(
                hdr,
                DerObjectContent::RelativeOID(Oid::from(&content))
            ))
    )
}
...
pub fn parse_der_explicit_failed(i: &[u8], tag: u8) -> IResult<&[u8], DerObject, u32> {
    value!(
        i,
        DerObject::from_obj(DerObjectContent::ContextSpecific(tag, None))
    )
}
...
where
    F: Fn(&[u8], u8, usize) -> IResult<&[u8], DerObjectContent, u32>,
{
    alt_complete!(
        i,
        do_parse!(
            hdr: der_read_element_header
                >> error_if!(hdr.tag != tag as u8, ErrorKind::Custom(DER_TAG_ERROR))
                >> content: map!(apply!(f, tag, hdr.len as usize), |b| DerObject::from_obj(b))
                >> (DerObject::from_header_and_content(
                    hdr,
                    DerObjectContent::ContextSpecific(tag, Some(Box::new(content)))
                ))
...
mod tests {
    ...
    fn test_der_bool() {
        ...
        assert_eq!(
            ...
            Err(Err::Error(error_position!(&[0x7f][..], ErrorKind::Verify)))
        );
    }
    ...
    fn test_der_int_long() {
        let empty = &b""[..];
        let bytes = [
            0x02, 0x0a, 0x39, 0x11, 0x45, 0x10, 0x94, 0x39, 0x11, 0x45, 0x10, 0x94,
        ];
    ...
    fn test_der_oid() {
        ...
        let expected = DerObject::from_obj(DerObjectContent::OID(Oid::from(&[
    ...
    fn test_der_seq() {
        ...
        let expected =
            DerObject::from_obj(DerObjectContent::Sequence(vec![DerObject::from_int_slice(
                b"\x01\x00\x01",
            )]));
    ...
    fn test_der_set() {
        let empty = &b""[..];
        let bytes = [
            0x31, 0x05, 0x02, 0x03, 0x01, 0x00, 0x01, // Integer 65537
        ];
        let expected = DerObject::from_obj(DerObjectContent::Set(vec![DerObject::from_int_slice(
            b"\x01\x00\x01",
        )]));
    ...
    fn test_der_utctime() {
        let empty = &b""[..];
        let bytes = [
            0x17, 0x0D, 0x30, 0x32, 0x31, 0x32, 0x31, 0x33, 0x31, 0x34, 0x32, 0x39, 0x32, 0x33,
            0x5A,
        ];
    ...
    fn test_der_generalizedtime() {
        let empty = &b""[..];
        let bytes = [
            0x18, 0x0D, 0x30, 0x32, 0x31, 0x32, 0x31, 0x33, 0x31, 0x34, 0x32, 0x39, 0x32, 0x33,
            0x5A,
        ];
        let expected = DerObject {
            class: 0,
    ...
    fn test_der_explicit() {
        ...
        let expected = DerObject {
            ...
            structured: 1,
            tag: 0,
            content: DerObjectContent::ContextSpecific(
                0,
                Some(Box::new(DerObject::from_int_slice(b"\x02"))),
            ),
        };
        assert_eq!(
            parse_der_explicit(&bytes, 0, parse_der_integer),
            Ok((empty, expected))
        );
        let expected2 = DerObject::from_obj(DerObjectContent::ContextSpecific(1, None));
        assert_eq!(
            parse_der_explicit(&bytes, 1, parse_der_integer),
            Ok((&bytes[..], expected2))
        );
    }
    ...
    fn test_der_implicit() {
        ...
        assert_eq!(
            parse_der_implicit(&bytes, 1, der_read_ia5string_content),
            Ok((empty, expected))
        );
        let expected2 = DerObject::from_obj(DerObjectContent::ContextSpecific(2, None));
        assert_eq!(
            parse_der_implicit(&bytes, 2, der_read_ia5string_content),
            Ok((&bytes[..], expected2))
        );
    }
    ...
    fn test_der_optional() {
        ...
        let expected1 = DerObject::from_obj(DerObjectContent::Sequence(vec![
            DerObject::from_obj(DerObjectContent::ContextSpecific(
                0,
        ...
        let expected2 = DerObject::from_obj(DerObjectContent::Sequence(vec![
            DerObject::from_obj(DerObjectContent::ContextSpecific(0, None)),
            DerObject::from_int_slice(b"\x01\x00\x01"),
        ]));
    ...
    fn test_der_seq_dn() {
        let empty = &b""[..];
        let bytes = [
            0x30, 0x45, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x46,
            0x52, 0x31, 0x13, 0x30, 0x11, 0x06, 0x03, 0x55, 0x04, 0x08, 0x0c, 0x0a, 0x53, 0x6f,
            0x6d, 0x65, 0x2d, 0x53, 0x74, 0x61, 0x74, 0x65, 0x31, 0x21, 0x30, 0x1f, 0x06, 0x03,
            0x55, 0x04, 0x0a, 0x0c, 0x18, 0x49, 0x6e, 0x74, 0x65, 0x72, 0x6e, 0x65, 0x74, 0x20,
            0x57, 0x69, 0x64, 0x67, 0x69, 0x74, 0x73, 0x20, 0x50, 0x74, 0x79, 0x20, 0x4c, 0x74,
            0x64,
        ];
        let expected = DerObject::from_obj(DerObjectContent::Sequence(vec![
            DerObject::from_obj(DerObjectContent::Set(vec![DerObject::from_obj(
    ...
    fn test_der_defined_seq_macros() {
        fn localparse_seq(i: &[u8]) -> IResult<&[u8], DerObject> {
            parse_der_sequence_defined_m!(i, parse_der_integer >> call!(parse_der_integer))
        }
        let empty = &b""[..];
        let bytes = [
            0x30, 0x0a, 0x02, 0x03, 0x01, 0x00, 0x01, 0x02, 0x03, 0x01, 0x00, 0x00,
    ...
    fn test_der_defined_set_macros() {
        fn localparse_set(i: &[u8]) -> IResult<&[u8], DerObject> {
            parse_der_set_defined_m!(i, parse_der_integer >> call!(parse_der_integer))
        }
        let empty = &b""[..];
        let bytes = [
            0x31, 0x0a, 0x02, 0x03, 0x01, 0x00, 0x01, 0x02, 0x03, 0x01, 0x00, 0x00,
