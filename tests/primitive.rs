...
fn test_flat_take() {
    ...
    assert_eq!(ber_read_element_content_as(&[0xff], BerTag::Boolean, 0x01), Ok((empty, BerObjectContent::Boolean(true))));
    assert_eq!(ber_read_element_content_as(&[0x00], BerTag::Boolean, 0x01), Ok((empty, BerObjectContent::Boolean(false))));
}
