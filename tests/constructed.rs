...
fn parse_struct01(i: &[u8]) -> IResult<&[u8], (DerObjectHeader, MyStruct)> {
    parse_der_struct!(
        i,
        a: parse_der_integer >> b: parse_der_integer >> (MyStruct { a: a, b: b })
    )
}
...
fn parse_struct01_complete(i: &[u8]) -> IResult<&[u8], (DerObjectHeader, MyStruct)> {
    parse_der_struct!(
        i,
        a: parse_der_integer >> b: parse_der_integer >> empty!() >> (MyStruct { a: a, b: b })
    )
}
...
fn parse_struct02(i: &[u8]) -> IResult<&[u8], (DerObjectHeader, ())> {
    parse_der_struct!(i, _a: parse_der_integer >> _b: parse_struct01 >> (()))
}
...
fn parse_struct03(i: &[u8]) -> IResult<&[u8], (DerObjectHeader, ())> {
    parse_der_struct!(
        i,
        _a: parse_der_integer >> _b: parse_der_struct!(parse_der_integer >> (())) >> (())
    )
}
        ...
        l: vec![
            ...
            Rdn {
                a: Attr {
                    ...
                    val: DerObject::from_obj(DerObjectContent::UTF8String(
